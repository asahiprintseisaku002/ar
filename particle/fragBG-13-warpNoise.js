...
float snoise(vec3 v) {
    ...
    vec3 x1 = x0 - i1 + C.xxx;
    vec3 x2 = x0 - i2 + C.yyy;
    vec3 x3 = x0 - 1.0 + vec3(C.x, C.x, C.x);
...
// ワープノイズを適用する関数
vec3 warpNoise(vec3 uv) {
    // 各成分に対して個別にノイズを適用
    float warpX = snoise(uv * 5.0 + vec3(0.0, 0.0, time * 0.2));  // X座標に基づくノイズ
    float warpY = snoise(uv * 5.0 + vec3(0.0, 1.0, time * 0.2));  // Y座標に基づくノイズ
    uv.xy += vec2(warpX, warpY) * 2.0;  // UV座標を変形させる
    float noiseValue = snoise(uv);  // 歪んだ座標で再度ノイズを計算
    return vec3(noiseValue);
}

void main() {
    // UV座標を生成
    vec2 uv = gl_FragCoord.xy / 5000.0;

    // ワープノイズを適用
    vec3 warpedColor = warpNoise(vec3(uv * 5.0, time * 1.0));
    ...
    // ワープノイズの値を色に反映させる
    mixedColor += warpedColor * 0.5;  // ワープによる揺らぎを加える
