...
void main() {
  ...
  // *** 法線の再計算を追加 ***
  // x方向とz方向に隣接する頂点の位置を少しずらして計算
  vec3 pos_dx = vec3(pos.x + 0.01, pos.y + sin((pos.x + 0.01) * 2.0 + time) * 0.1, pos.z);
  vec3 pos_dz = vec3(pos.x, pos.y + cos((pos.z + 0.01) * 10.0 + time) * 12.0, pos.z + 0.01);

  // 法線ベクトルを再計算（クロスプロダクトで求める）
  vec3 dx = pos_dx - pos;
  vec3 dz = pos_dz - pos;
  vNormal = normalize(cross(dx, dz));  // 法線ベクトルを再計算して正規化
