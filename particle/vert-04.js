...
void main() {
  ...
  // mouse.x, mouse.y は0から1なので、-125から+125の範囲でスケーリング
  vec3 mouse3D = vec3((mouse.x - 0.5) * 20.0, (mouse.y - 0.5) * 20.0, 0.0);  
  float distance = length(pos - mouse3D);  // パーティクルとカーソルの3D距離を計算
  float avoidRadius = 40.0;  // 半径250pxの球体
  ...
  // x座標にサイン波を使った左右揺れを追加
  pos.x += sin(time * 2.0 + position.y * 0.1) * 3.0;
