...
void main() {
  ...
  float avoidRadius = 150.0;  // カーソルの影響範囲（半径）

  if (distance < avoidRadius) {
    // 距離が近い場合、パーティクルをカーソルから押し出す（円形に押し出す）
    vec3 direction = normalize(mouse3D - pos);  // カーソルとパーティクルの方向を計算
    float strength = (avoidRadius - distance) / avoidRadius;  // 距離に応じて押し出す強さを調整
    pos += direction * strength * 300.0;  // パーティクルを外側に押し出す

    vSize = size * distance * 0.05;  // 距離に応じてサイズを小さくする
    gl_PointSize = max(vSize, 0.1);  // サイズが最小値を1に制限
  }
